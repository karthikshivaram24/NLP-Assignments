...
def make_feature_dicts(data,token=True,caps=True,pos=True,chunk=True,context=True):
    """
...
def evaluate_combinations(train_data, test_data):
    ...
    """
    settings = []
    combos = sorted(list(product([True,False],repeat=4)))
    result = pd.DataFrame(0,index = [x for x in range(0,len(combos))],columns=["f1","n_params","caps","pos","chunk","context"])
    for i in range(len(combos)):
        X_train_dic,X_train_labels = make_feature_dicts(data=train_data,token=True,caps=combos[i][0],pos=combos[i][1],chunk=combos[i][2],context=combos[i][3])
        vectorizer = DictVectorizer()
        X_TRAIN = vectorizer.fit_transform(X=X_train_dic)
        X_test_dic,X_test_labels = make_feature_dicts(data=test_data,token=True,caps=combos[i][0],pos=combos[i][1],chunk=combos[i][2],context=combos[i][3])
